...
const PRESETS = {
  default:{
    ...
    glass:'off', mode:'system', font:'system', ambientAnimation:'off'
  },
  gold:{
    ...
    glass:'on', mode:'dark', font:'serif', luxClass:'lux-gold', ambientAnimation:'off'
  },
  noir:{
    ...
    glass:'off', mode:'dark', font:'modern', luxClass:'lux-noir', ambientAnimation:'off'
  },
  rose:{
    ...
    glass:'on', mode:'dark', font:'rounded', luxClass:'lux-rose', ambientAnimation:'off'
  },
  emerald:{
    ...
    glass:'on', mode:'dark', font:'modern', luxClass:'lux-emerald', ambientAnimation:'off'
  }
};

export function getPreset(name){ return PRESETS[name] ? structuredClone(PRESETS[name]) : null; }
...
export function mergePresetIntoSettings(name, settings){
  ...
  // do not force ambientAnimation from preset (keep user choice) unless preset defines
  if (p.ambientAnimation) settings.ambientAnimation=p.ambientAnimation;
  return settings;
}

export function applyTheme(settings){
  if (!settings) return;
  const { storeName, tagline, colors, background, glass, logoEmoji, logoDataUrl, font, luxClass, ambientAnimation } = settings;
  ...
  if (colors){
    ...
    root.style.setProperty('--color-accent-soft', rgba(colors.primary,0.18));
  ...
  document.body.classList.toggle('ambient-anim', ambientAnimation==='on');
}
...
function blend(a,b,t){ const pa=hexToRgb(a), pb=hexToRgb(b);
...
export function exportThemeOnly(){
  ...
  return JSON.stringify({
    ...
    luxClass:s.luxClass||'',
    ambientAnimation:s.ambientAnimation||'off'
  }, null, 2);
}
